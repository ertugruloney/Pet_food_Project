...
orderamounts=m.addVars(product,weeks,name="orderamounts",lb=0,vtype="C")
InventorySupp=m.addVars(product,weeks,name="Inventory at supplier",lb=0,vtype="C")
InventoryWare=m.addVars(product,weeks,name="Inventory in warehouse",lb=0,vtype="C")
ShipmentFWare=m.addVars(product,weeks,name="Shipments from supplier to WH",lb=0,vtype="C")
...
#add constarait
constraintadd= m.addConstrs(orderamounts[i,j]<=300000 for j in range(weeks) for i in  range(product));
